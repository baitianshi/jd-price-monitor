...
function record_login_failure($db) {
    ...
    if ($result) {
        ...
        if ($newAttempts >= LOGIN_MAX_ATTEMPTS) {
            $db->execute(
                "UPDATE login_attempts SET attempts = ?, lockout_until = datetime('now', 'localtime', '+15 minutes'), last_attempt = datetime('now', 'localtime') WHERE ip_address = ?",
                [$newAttempts, $ip]
            );
        } else {
            $db->execute(
                "UPDATE login_attempts SET attempts = ?, last_attempt = datetime('now', 'localtime') WHERE ip_address = ?",
                [$newAttempts, $ip]
            );
        }
    } else {
        $db->execute(
            "INSERT INTO login_attempts (ip_address, attempts, last_attempt) VALUES (?, 1, datetime('now', 'localtime'))",
            [$ip]
        );
    }
}
