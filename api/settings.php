...
function handleGet($db, $jd) {
    ...
    // 更新数据库中的Cookie状态
    $db->execute("UPDATE settings SET cookie_status = ?, updated_at = datetime('now', 'localtime') WHERE id = 1", [$cookieStatus]);
    ...
    $stats = $db->fetch("
        SELECT 
            ...
            SUM(CASE WHEN status = 'active' AND current_price > 0 AND target_price > 0 AND current_price <= target_price THEN 1 ELSE 0 END) as price_met_products,
...
function handlePost($db) {
    ...
    if (isset($data['history_retention_days'])) {
        $updateFields[] = "history_retention_days = ?";
        $updateValues[] = intval($data['history_retention_days']);
    }
