...
function checkSingleProduct($db, $jd, $webhook, $id) {
    ...
    if ($newPrice <= 0) {
        return [
            'success' => false,
            'message' => $productInfo['error_message'] ?? '无法获取价格',
            'error_type' => $productInfo['error_type'] ?? 'unknown',
            'status' => $productInfo['status'] ?? 'error'
        ];
    }
...
function checkNotifications($db, $webhook, $product, $oldPrice, $newPrice, $oldStockStatus, $newStockStatus, $lowestPrice) {
    $notifications = [];
    $nextCheckTime = date('Y-m-d H:i:s', strtotime('+30 minutes'));
