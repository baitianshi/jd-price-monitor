...
function handlePost($db, $jd) {
    ...
    // 获取商品信息（跳过图片，异步加载）
    $productInfo = $jd->getProductInfo($skuId, true);
    ...
    json_success([
        ...
        'error_type' => $productInfo['error_type'] ?? '',
        'error_message' => $productInfo['error_message'] ?? '',
    ], '添加成功');
}
...
function handlePut($db) {
    ...
    $allowedFields = [
        'name', 'target_price', 'price_change_threshold',
        'tags', 'notify_price_drop', 'notify_lowest', 'notify_price_surge',
        'notify_oos', 'status', 'history_retention_days'
    ];
    ...
    $updateFields[] = "updated_at = datetime('now', 'localtime')";
    $updateValues[] = $id;
 ...
 * 异步加载商品图片
 */
function handleLoadImage($db, $jd) {
    $id = $_GET['id'] ?? null;
    $skuId = $_GET['sku_id'] ?? null;
    
    if (!$skuId && $id) {
        $product = $db->fetch("SELECT sku_id FROM products WHERE id = ?", [$id]);
        if ($product) {
    ...
    $imageUrl = $jd->getProductImage($skuId);
    
    // 更新数据库
    if ($imageUrl && $id) {
        $db->execute(
            "UPDATE products SET image_url = ?, updated_at = datetime('now', 'localtime') WHERE id = ?",
            [$imageUrl, $id]
        );
    }
